...
import (
	...
	stdgen "github.com/husnulhamidiah/goyoga/internal/std-gen"
)
...
func main() {
	...
	out := flag.String("out", "std", "output directory")
	flag.Parse()
	...
	if err := stdgen.Generate(api, *out); err != nil {
		fatal(err)
	}
	fmt.Printf("generated std api from %d enums, %d typedefs, %d functions\n",
		len(api.Enums), len(api.Typedefs), len(api.Functions))
}
